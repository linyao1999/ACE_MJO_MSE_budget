...
def get_local_MSE(ds, lat_lim=50, latmean=False, iceflg=False):
    ...
    if latmean:
        ...
        qv = ds['QLV'].sel(lat=slice(-lat_lim, lat_lim)).mean('lat').load()

        mse = cp * T + g * z + qv
    else:
        T = ds['T'].sel(lat=slice(-lat_lim, lat_lim)).load()
        z = ds['Z3'].sel(lat=slice(-lat_lim, lat_lim)).load()
        qv = ds['QLV'].sel(lat=slice(-lat_lim, lat_lim)).load()
...
def get_local_MSE_tendency_sep(dse, qlv, lat_lim=50, latmean=False, iceflg=False):

    ddse_dt = xr.full_like(dse, fill_value=np.nan)  # [time, lev, lat, lon]
    ddse_dt[1:-1, :, :, :] = (dse[2:, :, :, :].values-dse[:-2, :, :, :].values) / 2
    ddse_dt[0, :, :, :] = dse[1, :, :, :].values - dse[0, :, :, :].values
    ddse_dt[-1, :, :, :] = dse[-1, :, :, :].values - dse[-2, :, :, :].values

    dqlv_dt = xr.full_like(qlv, fill_value=np.nan)  # [time, lev, lat, lon]
    dqlv_dt[1:-1, :, :, :] = (qlv[2:, :, :, :].values-qlv[:-2, :, :, :].values) / 2
    dqlv_dt[0, :, :, :] = qlv[1, :, :, :].values - qlv[0, :, :, :].values
    dqlv_dt[-1, :, :, :] = qlv[-1, :, :, :].values - qlv[-2, :, :, :].values

    return ddse_dt, dqlv_dt
...
def get_local_MSE_budget(ds, lat_lim=50, plim=100, latmean=False, iceflg=False, source='target'):
    mse_budget = {}
    ...
    if source == 'target':
        dtmse_sel = get_local_MSE_tendency(ds, lat_lim=lat_lim, latmean=latmean).load().copy() # [time, lev, lat, lon]
        raw_dtmse = dtmse_sel.sel(lev=slice(plim,None))
        mse_budget['tendency'] = raw_dtmse * 86400  # J/kg/day
    else:
        # calcualte time derivative of MSE by the difference of two steps of MSE 
        mse = mse_budget['mse'].copy()
        dmse_dt = xr.full_like(mse, fill_value=np.nan)  # [time, lev, lat, lon]

        dmse_dt[1:-1, :, :, :] = (mse[2:, :, :, :]-mse[:-2, :, :, :]) / 2
        dmse_dt[0, :, :, :] = mse[1, :, :, :] - mse[0, :, :, :]
        dmse_dt[-1, :, :, :] = mse[-1, :, :, :] - mse[-2, :, :, :]

        mse_budget['tendency'] = dmse_dt.copy()  # J/kg/day
...
def get_local_MSE_budget_sep(ds, lat_lim=50, latmean=False, iceflg=False):
    mse_budget = {}

    # get local MSE
    raw_dse, raw_qlv = get_local_MSE_sep(ds, lat_lim=lat_lim, latmean=latmean)  # [time, lev, lat, lon]

    mse_budget['dse'] = raw_dse
    mse_budget['qlv'] = raw_qlv
    print('get local MSE')

    raw_dtdse, raw_dtqlv = get_local_MSE_tendency_sep(raw_dse, raw_qlv, lat_lim=lat_lim, latmean=latmean) # [lev, lat, lon]

    mse_budget['dtdse'] = raw_dtdse   # J/kg/day
    mse_budget['dtqlv'] = raw_dtqlv 
    print('get local MSE tendency')

    # get local MSE source: CRM_ALT
    # raw_dtdse = (get_local_MSE_source(ds,'DDSE_CRM_ALT',lat_lim=lat_lim, latmean=latmean).load()
    ...
    raw_dtdse = (get_local_MSE_source(ds,'DDSE_convection',lat_lim=lat_lim, latmean=latmean).load())
    raw_dtqlv = get_local_MSE_source(ds,'DQLV_CRM_ALT',lat_lim=lat_lim, latmean=latmean).load()
    ...
    # get local MSE source: PBL
    raw_dtdse = get_local_MSE_source(ds,'DDSE_PBL',lat_lim=lat_lim, latmean=latmean).load()
    raw_dtqlv = get_local_MSE_source(ds,'DQLV_PBL',lat_lim=lat_lim, latmean=latmean).load()
    ...
    raw_dtdse = (get_local_MSE_source(ds,'DDSE_QRS',lat_lim=lat_lim, latmean=latmean).load()
                 + get_local_MSE_source(ds,'DDSE_QRL',lat_lim=lat_lim, latmean=latmean).load())
    ...
    raw_dtdse = get_local_MSE_source(ds,'DDSE_DYN',lat_lim=lat_lim, latmean=latmean).load()
    raw_dtqlv = get_local_MSE_source(ds,'DQLV_DYN',lat_lim=lat_lim, latmean=latmean).load()
